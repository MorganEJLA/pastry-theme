...
(function () {
  ...
  const suggestionsBox = browseSection.querySelector("#locale-inline-suggestions");
  ...
  const locales = Array.from(cards).map(card => {
    ...
    const label = titleEl ? titleEl.textContent.trim() : card.dataset.title || "";
  ...
  function renderSuggestions(list) {
    ...
    suggestionsBox.querySelectorAll(".locale-search__suggestion").forEach(btn => {
      btn.addEventListener("click", () => {
  ...
  searchInput.addEventListener("input", () => {
    ...
    const matches = locales.filter(locale => locale.title.includes(q)).slice(0, 8); // cap suggestions
  ...
  searchInput.addEventListener("keydown", e => {
    if (e.key === "Enter") {
      const first = suggestionsBox ? suggestionsBox.querySelector(".locale-search__suggestion") : null;
      if (first) {
        e.preventDefault();
  ...
  constructor() {
    if (document.querySelector(".hero-slider")) {
      ...
      for (let i = 0; i < dotCount; i++) {
        dotHTML += `<button class="slider__bullet glide__bullet" data-glide-dir="=${i}" aria-label="Slide ${i + 1}"></button>`;
      }
...
class Search {
  ...
  async getResults() {
    try {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().get(`${pastryData.root_url}/wp-json/pastry/v1/search?term=${this.searchField.value}`);
      ...
      this.resultsDiv.innerHTML = `
        <div class="row">
          ...
          <div class="one-third">
            ...
            ${results.event.length ? '<ul class="link-list min-list">' : "<p>No featured desserts match that search.</p>"}
            ...
            ${results.pastry_case.length ? '<ul class="link-list min-list">' : "<p>No pastry case items match that search.</p>"}
              ${results.pastry_case.map(item => `
                    <li>
                      <a href="${item.permalink}">${item.title}</a>
                    </li>
                  `).join("")}
          ...
          <div class="one-third">
            ...
            ${results.locale.length ? '<ul class="link-list min-list">' : "<p>No locales match that search.</p>"}
              ${results.locale.map(item => `
                    <li>
                      <a href="${item.permalink}">${item.title}</a>
                    </li>
                  `).join("")}
            ...
            ${results.journal.length ? '<ul class="link-list min-list">' : "<p>No journal entries match that search.</p>"}
  ...
  keyPressDispatcher(e) {
    if (e.keyCode === 83 &&
    // "s"
    !this.isOverlayOpen && document.activeElement.tagName !== "INPUT" && document.activeElement.tagName !== "TEXTAREA") {
      this.openOverlay();
    }
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = globalThis["webpackChunkpastry_theme"] = globalThis["webpackChunkpastry_theme"] || [];
